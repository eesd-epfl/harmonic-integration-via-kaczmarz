...
class L2Solvers:
    ...
    @staticmethod
    def solve_2_norm_Ridge(D_mat, v, alpha=0.1):
        t1 = time.time()
        X = []
        N = np.shape(D_mat)[1]
        for i in range(0, N):
    ...
    @staticmethod
    def solve_2_norm_inverse_proj(D_mat, v):
    ...
    @staticmethod
    def solve_2_norm_CVX(D_mat, v, coefs_star=[]):
        # usage example: coefs_cvx = solve_2_norm_CVX(D_mat.T, v.T)
        n_max = int(D_mat.shape[1])
        print(n_max)
        print(D_mat.shape)
        print(v[:, 0].shape)
        #if coefs_star == []: coefs_star = np.random.randn(v.shape[0], v.shape[1])
        t1 = time.time()
        x = cp.Variable(n_max, complex=True)
        cost_x = cp.sum_squares(D_mat @ x - v[:, 0].T)
        prob_x = cp.Problem(cp.Minimize(cost_x))
        prob_x.solve()
    
        y = cp.Variable(n_max, complex=True)
        cost_y = cp.sum_squares(D_mat @ y - v[:, 1].T)
        prob_y = cp.Problem(cp.Minimize(cost_y))
        prob_y.solve()
    
        z = cp.Variable(n_max, complex=True)
        cost_z = cp.sum_squares(D_mat @ z - v[:, 2].T)
        prob_z = cp.Problem(cp.Minimize(cost_z))
        prob_z.solve()
        t2 = time.time()-t1
        print("\nSolver's time: {} min".format(str(t2/60)))
        coefs[:, 0] = prob_x.value; coefs[:, 0] = prob_y.value; coefs[:, 2] = prob_z.value
        return coefs
...
def shape_descriptors(coefs):
    # Please watch out the dimensions and the assumptions made here, if it doesn't work please take the transpose of the inputed matrix
    max_n = int(np.sqrt(coefs.shape[0])-1)
    Dr_x = np.zeros((max_n,))
    Dr_y = np.zeros((max_n,))
    Dr_z = np.zeros((max_n,))
    # Rotation-invariant
